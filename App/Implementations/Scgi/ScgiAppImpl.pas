...
type
    ...
    (*!-----------------------------------------------
     * SCGI web application that implements IWebApplication
     *
     * @author Zamrony P. Juhara <zamronypj@yahoo.com>
     *-----------------------------------------------*)
    TScgiWebApplication = class(TFanoWebApplication, IDataAvailListener, IReadyListener)
    private
...
resourcestring
     ...
     * default service provider
     *-----------------------------------------------*)
    constructor TScgiWebApplication.create(
        const container : IDependencyContainer;
        const errHandler : IErrorHandler;
        const dispatcherInst : IDispatcher;
        const server : IRunnableWithDataNotif;
    ...
    destructor TScgiWebApplication.destroy();
    begin
        inherited destroy();
        dispatcher := nil;
        workerServer := nil;
     ...
     * Note that run keeps run until application is terminated
     *-----------------------------------------------*)
    procedure TScgiWebApplication.attachListenerAndRunServer();
    begin
     ...
     * Note that run keeps run until application is terminated
     *-----------------------------------------------*)
    function TScgiWebApplication.run() : IRunnable;
    begin
     ...
     * @param env, CGI environment
     *-----------------------------------------------*)
    procedure TScgiWebApplication.executeRequest(const env : ICGIEnvironment);
    begin
        try
            environment := env;
            execute();
     ...
     * @return true if data is handled
     *-----------------------------------------------*)
    function TScgiWebApplication.handleData(
        const stream : IStreamAdapter;
        const context : TObject;
        const streamCloser : ICloseable
    ) : boolean;
     ...
     * @return true request is handled
     *-----------------------------------------------*)
    function TScgiWebApplication.ready(
        const socketStream : IStreamAdapter;
        const env : ICGIEnvironment;
        const stdInStream : IStreamAdapter
    ) : boolean;
